...
if __name__ == "__main__":
    ...
    model: KeypointNetwork = model.load_from_checkpoint(
        checkpoint_path='/home/kanishk/Desktop/keypointnet-resnet18-nc32.ckpt',
    ...
    for j in tqdm(range(3), desc="Benchmarking AUC of PCK@k", total=3):
        ...
        for k in tqdm(range(1, 100), desc=f"Iteration ~ {j}", total=99):
            ...
            descriptor_a = model.get_dense_representation(rgb_a.permute(2, 0, 1).unsqueeze(0)).squeeze(dim=0).to(device)
            ...
            descriptor_b = model.get_dense_representation(rgb_b.permute(2, 0, 1).unsqueeze(0)).squeeze(dim=0).to(device)
            ...
            AUC.append(pck)
        print(AUC)
        AUCS_log.append(torch.hstack(AUC).detach().cpu().numpy())
    ...
    np.savetxt("sandbox/keypointnet/AUC_log.txt", np.vstack(AUCS_log))
