...
if __name__ == "__main__":
    ...
    # Load object
    objs = bproc.loader.load_obj("dataset/models/30be60f95e2a3b47a6e67635b9d1cee5/models/model_normalized.obj")
    object = objs[0]
    ...
    for i in range(100):
        ...
        random_transformation = np.random.uniform([-0.5, -0.5, 4], [0.5, 0.5, 5])
    ...
    for i, image in enumerate(data["colors"]):
        cv2.imwrite(f"dataset/rgbs/{i + 100}_rgb.png", image)

    for i, depth in enumerate(data["depth"]):
        np.save(f"dataset/depths/{i + 100}_depth", depth)

    for i, mask in enumerate(data["instance_segmaps"]):
        mask = np.array(mask)
        cv2.imwrite(f"dataset/masks/{i + 100}_mask.png", generate_erode_mask(mask))

    for i, pose in enumerate(list_of_poses):
        np.savetxt(f"dataset/cam_to_world_trafo/{i + 100}_pose.txt", np.array(pose))
